...
class App {
  ...
  private route(): void {
    Object.keys(routes).forEach((key: string) => {
      this.app.use(key, Container.resolve<Router>(routes[key]))
    });
  }

  private globalRoute(): void {
    this.app.all([ '/' ], (_req: Request, res: Response): OutgoingMessage => res.status(status.OK).json(apiResponse({
      stat_code: status.OK,
      stat_message: 'Hello World!'
    })))
  }
